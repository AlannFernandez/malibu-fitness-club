...
export default function RoutineBuilder({ onBack, onLogout }: RoutineBuilderProps) {
  ...
  const [errorLoadingExercises, setErrorLoadingExercises] = useState<string | null>(null);


  useEffect(() => {
    const fetchExercises = async () => {
      setIsLoadingExercises(true);
      setErrorLoadingExercises(null);
      try {
        const fetched = await exerciseService.getExercises();
        if (fetched) {
          const transformedExercises: Exercise[] = fetched.map((ex: any) => ({
            id: ex.id,
            name: ex.name,
        ...
        setErrorLoadingExercises(`Error al cargar ejercicios: ${error.message || 'Error desconocido'}`);
      } finally {
  ...
  const handleDrop = (e: React.DragEvent, dayId: string) => {
    e.preventDefault()
    if (draggedExercise) {
      // Crea una copia profunda del ejercicio y asigna un ID único para evitar problemas de claves duplicadas
                ...
                <CardContent className="p-0">
                  <ScrollArea className="h-100vh">
                    {isLoadingExercises ? (
                        ...
                        <div className="p-4 text-center text-red-400">{errorLoadingExercises}</div>
